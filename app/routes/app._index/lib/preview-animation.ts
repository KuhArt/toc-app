import type { PreviewTocItem, TocAnimationType } from "./types";

export type TocSnakeGeometry = {
  headAngle: number;
  headBend: number;
  headX: number;
  headY: number;
...
export type TocJumpingMarkerFlight = {
...
export type TocSnakeClickFlight = {
  duration: number;
...
export const TOC_CRAWLING_SNAKE_VISIBLE_LENGTH = 10;
...
export function getMarkerSettingsForList(list: HTMLUListElement): TocMarkerSettings {
  const widget = list.closest(".toc-widget");
...
export function isDesktopMarkerAnimation(
...
export function isFollowingMarkerAnimation(animationType: TocAnimationType) {
  return animationType === "following-marker";
}

export function isCrawlingSnakeAnimation(animationType: TocAnimationType) {
  return animationType === "crawling-snake";
}

export function isJumpingMarkerAnimation(animationType: TocAnimationType) {
  return animationType === "jumping-marker";
}

export function getPreviewReplayStepDelay(animationType: TocAnimationType) {
...
export function flattenPreviewItemIds(items: PreviewTocItem[]): string[] {
...
export function buildPreviewReplaySequence(itemIds: string[], maxDepth: number) {
...
export function getJumpingMarkerProgress(
...
export function snapRotationToQuarterTurn(angle: number) {
  return Math.round(angle / 90) * 90;
}

export function measureListLinkHeadPoint(
...
export function buildJumpingMarkerFlight(
  list: HTMLUListElement,
  startPoint: TocPoint,
  startRotation: number,
...
export function buildSnakeClickFlight(
  list: HTMLUListElement,
...
export function getSnakeClickFlightProgress(
  flight: TocSnakeClickFlight,
  now: number,
) {
...
export function getSnakeFlightCurrentLinkId(
  list: HTMLUListElement,
  fromLink: HTMLAnchorElement | null,
  toLink: HTMLAnchorElement | null,
  progress: number,
...
export function measureTocSnakeGeometry(
  list: HTMLUListElement,
...
export function measureTocJumpingMarkerGeometry(
  list: HTMLUListElement,
  activeLink: HTMLAnchorElement | null,
...
export function measureTocSnakeClickFlightGeometry(
  list: HTMLUListElement,
  fromLink: HTMLAnchorElement | null,
  toLink: HTMLAnchorElement | null,
  progress: number,
...
export function snakeGeometryEqual(
...
function chooseParabolaControlPoint(
  ...
  const preferredRoom = measureRayToBounds(midpoint, preferredDirection, bounds);
...
function buildSettledSnakePoints(metrics: TocSnakeLinkMetric[]) {
  ...
  metrics.forEach((metric, index) => {
    const turnY =
      index === 0 ? metric.entryY : (metrics[index - 1].exitY + metric.entryY) / 2;
